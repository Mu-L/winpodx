...
- **`winpodx doctor --fix` 가 흔한 복구 가능 상태를 자동 교정 (0.6.0 항목 K).** `doctor` 는 이미 문제를 보고하고 명령을 제안했는데, `--fix` 는 이제 fix 가 등록된 warn/fail finding 마다 안전·멱등 교정을 실행하고 `doctor` 재실행으로 검증하라고 안내. fix 없는 finding 은 보고만 하고 그대로 두며, 실패한 fix 는 (치명적 아님) 보고하고 나머지 fix 는 계속 실행 — `FAIL` finding 의 fix 도 doctor 의 non-zero exit 전에 실행됨. 이번 릴리스 fix: persisted 앱 목록에서 누락/stale 데스크탑 엔트리 재등록(게스트 왕복 없음 — pod 꺼져 있어도 동작), dangling 트레이 자동시작 엔트리 제거(`winpodx` 바이너리 사라진 경우), 반쯤 끝난 설치 resume(항목 B 의 통합 resume 경로에 위임). 새 `doctor` 검사 **desktop-entries** 가 DE 메뉴에 안 뜬 discovered 앱을 표면화. `docs/design/ROADMAP-0.6.0.md` 항목 K 참조.
