...
- **Linux 앱을 Windows URL 스킴의 처리기로 등록할 수 있습니다** (#798, #694). Windows 게스트 안에서 `mailto:`나 사용자 지정 스킴 링크를 클릭하면, 이미 파일을 처리하던 것과 같은 제어된 리스너를 통해 대응하는 호스트 앱으로 전달됩니다 (#796). Windows 앱 안의 링크로 Linux 메일 클라이언트나 브라우저를 열 수 있습니다.
- **이제 Windows가 호스트 언어로 설치됩니다** (#804, #791, #790). 영어 기본값 대신 데스크톱 설정에서 설치 로케일을 가져오며, `install locale`이 비어 있으면 "en-US"가 아니라 "자동 감지"를 뜻합니다 (#806).
- **미디어 드라이브 리디렉션을 끌 수 있습니다** (#840). 이동식 미디어를 게스트에 노출하고 싶지 않은 환경을 위해 RDP 계층에 옵트아웃을 추가했고, 문서와 배포되는 설정 예제에도 반영했습니다.
...
- **`winpodx install`이 몇 분씩 조용한 두 구간을 미리 알립니다** (#805, #789). 새 설치에서 ISO 다운로드와 OEM 단계가 멈춘 것처럼 보이지 않습니다.
- **FreeRDP 처리를 하나로 모았습니다**: 버전 탐지를 공유하고 RemoteApp을 망가뜨리는 구버전 RAIL을 실행 시점에 경고하며 (#797, #785), auto 모드에서 구버전보다 현재 네이티브 FreeRDP를 우선합니다 (#702).
...
- **데스크톱 항목**: `Exec=` 줄이 절대 경로의 `winpodx`를 사용하고 (#795, #779), PNG 아이콘을 실제 hicolor 크기 디렉터리에 배치합니다 (#803, #702).
- **bring-up 라이브 로그에 dockur 상태 줄이 잘리지 않고 그대로 남습니다** (#842, @rruxx와 crux).
- **직접 선택하지 않으면 Windows 검색 인덱서를 건드리지 않습니다** (#802, #570).
- **`winpodx doctor`가 앱 버전 차이를 OEM 드리프트로 보고하지 않고** (#827), 복구 중에도 MIME 기본값을 옵트인으로 유지합니다 (#820).
- **깨진 RemoteApp IME 동기화를 차단해** 원격 앱 창에서 CJK 입력이 깨지지 않습니다 (#815).
- **중국어 간체 카탈로그를 완성했습니다** (#792, @zkitefly).
- **디블로트 스크립트의 안전 범위를 테스트로 고정해** (#845, @GameSoul7Eugene) 위험한 예약 작업 확장과 광고/위젯 레지스트리 변경을 거부합니다.
- **GUI 디블로트 선택기 스타일이 안정화됐습니다** (#813).
- Star history 차트를 외부 서비스에서 가져오지 않고 저장소에서 직접 제공합니다 (#861, #862).

### Contributors

이번 릴리즈에는 다음 분들의 기여가 담겼습니다:
...
- **@GameSoul7Eugene** — 디블로트 안전 범위 회귀 테스트 (#845), Dashboard 색상 수정의 계기가 된 제보 (#811, #812)
- **@rruxx**, **crux**와 함께 — 라이브 로그에 dockur 상태 줄 전체 유지 (#842)
- **@zkitefly** — 중국어 간체 카탈로그 완성 (#792)

여기서 고친 이슈를 제보해 주신 모든 분께도 감사드립니다: #570, #694, #702, #730, #735, #770, #779, #785, #789, #790, #791, #811, #812, #813, #815, #820, #827, #836, #840, #843, #844, #852.
